...
export const postLogin = async (req, res) => {
    ...
    try {
        ...
        if(typeof rows[0] ==='undefined' || !(await bcryptjs.compare(password, rows[0].contraseña))){
            ...
            res.cookie('idUser', rows[0].usuarioid, { maxAge: 900000, httpOnly: true, secure: true })
            /* req.session.rol = rows[0].rol */
            res.cookie('nombres', result1.rows[0].nombres, { maxAge: 900000, httpOnly: true, secure: true })
            res.cookie('apellidos', result1.rows[0].apellidos, { maxAge: 900000, httpOnly: true, secure: true })
            res.cookie('foto_url', result1.rows[0].foto_url, { maxAge: 900000, httpOnly: true, secure: true})
